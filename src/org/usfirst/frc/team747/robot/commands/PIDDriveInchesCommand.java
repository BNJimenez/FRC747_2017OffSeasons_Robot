...
//      this.driveTicks = inches / ENCODER_TICKS_PER_REVOLUTION;
        ...
        if (reverse) {
            this.driveTicks = -Robot.DRIVE_TRAIN.applyGearRatio(Robot.DRIVE_TRAIN.convertInchesToRevs(inches * ENCODER_TICKS_PER_REVOLUTION));//input now has to be ticks instead of revolutions which is why we multiply by 4096
        } else {
            this.driveTicks = Robot.DRIVE_TRAIN.applyGearRatio(Robot.DRIVE_TRAIN.convertInchesToRevs(inches * ENCODER_TICKS_PER_REVOLUTION));
        }
